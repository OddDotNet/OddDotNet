...
public class AiRequestShorthandBuilder : IShorthandFilterBuilder<RequestQueryRequest>
{
    private const string Signal = "appinsights/requests";

    public RequestQueryRequest Build(IQueryCollection query)
    {
        var request = new RequestQueryRequest
        {
        ...
        foreach (var (name, values) in query)
        {
            ...
            switch (name)
            {
                ...
                case "response_code": pf.ResponseCode = Eq(value); break;
                case "url":           pf.Url = Eq(value); break;
                case "source":        pf.Source = Eq(value); break;
